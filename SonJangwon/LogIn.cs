...
namespace BookStore
{
    class LogIn
    {
        ...
        public void Rent()
        {
            ...
            Console.WriteLine("1. 책 이름으로 선택하기 2.저자 이름으로 선택하기");
            ChooseNum = exception.exceptionnum(2);                                  //예외처리입니다.
            switch (ChooseNum)
            {
                case 1:
                    while (true)
                    ...
                    switch (select)
                    {
                        case 1:
                            ...
                            Console.WriteLine("{0} 가 {1}에 대여되었습니다.", menu.getAdmin().bookVolist[i].BookName, RentTime[RentTime.Count - 1]);
